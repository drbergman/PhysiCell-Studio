...
def invertf2s(
    sval,
):  # takes a numeric string, converts to float, returns string with num_dec decimal places.
    num_dec = 5
    if (
        float(sval) < 1.0e-6
    ):  # e.g., necrosis duration=0 for index=0 (in biorobots, etc)
        return "9.e9"
    else:
...
def populate_tree_cell_defs(cell_def_tab, skip_validate):
    ...
    if uep:
        ...
        for cell_def in uep:
            ...
            if cell_def.tag != "cell_definition":
                logging.debug(
                    f"-------- found unexpected child <cell_definitions>; skip over {cell_def}"
                )
                continue
            if cell_def.tag == "cell_rules":
                logging.debug(
                    f"-------- found cell_rules child; break out on {cell_def}"
                )
                break
            ...
            cell_def_tab.init_default_phenotype_params(
                cell_def_name, False
            )  # rwh 8/23/23: reset_mapping flag
            # print("populate_(): ",cell_def_tab.param_d)
            # print("------   after populate ----------\n\n")

            cell_def_tab.param_d[cell_def_name]["ID"] = cell_def.attrib[
                "ID"
            ]  # e.g., "0"
            # cell_def_tab.param_d[cell_def_name]["name"] = cell_def_name
            cell_def_tab.current_cell_def = (
                cell_def_name  # do this for the callback methods?
            )
            ...
            cell_def_tab.tree.resizeColumnToContents(
                idx
            )  # rwh (after adding ID column)
            ...
            cell_def_tab.param_d[cell_def_name][
                "cycle_choice_idx"
            ] = cell_def_tab.cycle_dropdown.currentIndex()
                ...
                for rate in pt_uep:
                    ...
                    if rate.attrib["start_index"] == "0":
                        if rate.attrib["end_index"] == "0":  #  Must be 'live'
                            ...
                            cell_def_tab.param_d[cell_def_name][
                                "cycle_live_duration00"
                            ] = invertf2s(
                                sval
                            )  # invert
                            if rate.attrib["fixed_duration"].lower() == "true":
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_live_trate00_fixed"
                                ] = True
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_live_duration00_fixed"
                                ] = True
                        elif rate.attrib["end_index"] == "1":
                            if cycle_code == 0:  #'advanced Ki67'
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_advancedKi67_trate01"
                                ] = sval
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_advancedKi67_duration01"
                                ] = invertf2s(
                                    sval
                                )  # invert
                                if rate.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_advancedKi67_trate01_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_advancedKi67_duration01_fixed"
                                    ] = True
                            elif cycle_code == 1:  # 'basic Ki67'
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_Ki67_trate01"
                                ...
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_Ki67_duration01"
                                ] = invertf2s(
                                    sval
                                )  # invert
                                if rate.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_Ki67_trate01_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_Ki67_duration01_fixed"
                                    ] = True
                            elif cycle_code == 2:  # 'flow cytometry'
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_flowcyto_trate01"
                                ] = sval
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_flowcyto_duration01"
                                ] = invertf2s(
                                    sval
                                )  # invert
                                if rate.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_flowcyto_trate01_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_flowcyto_duration01_fixed"
                                    ] = True
                            elif cycle_code == 6:  # 'flow cytometry separated'
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_flowcytosep_trate01"
                                ] = sval
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_flowcytosep_duration01"
                                ] = invertf2s(
                                    sval
                                )  # invert
                                if rate.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_flowcytosep_trate01_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_flowcytosep_duration01_fixed"
                                    ] = True
                            elif cycle_code == 7:  # 'cycling quiescent'
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_quiescent_trate01"
                                ] = sval
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_quiescent_duration01"
                                ] = invertf2s(
                                    sval
                                )  # invert
                                if rate.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_quiescent_trate01_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_quiescent_duration01_fixed"
                                    ] = True
                        ...
                        if rate.attrib["end_index"] == "0":
                            if cycle_code == 1:  # 'basic Ki67'
                                ...
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_Ki67_duration10"
                                ] = invertf2s(
                                    sval
                                )  # invert
                                if rate.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_Ki67_trate10_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_Ki67_duration10_fixed"
                                    ] = True
                            elif cycle_code == 7:  # 'cycling quiescent'
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_quiescent_trate10"
                                ] = sval
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_quiescent_duration10"
                                ] = invertf2s(
                                    sval
                                )  # invert
                                if rate.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_quiescent_trate10_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_quiescent_duration10_fixed"
                                    ] = True
                        elif rate.attrib["end_index"] == "2":
                            if cycle_code == 0:  #'advanced Ki67'
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_advancedKi67_trate12"
                                ] = sval
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_advancedKi67_duration12"
                                ] = invertf2s(
                                    sval
                                )  # invert
                                if rate.attrib["fixed_duration"].lower() == "true":
                                    ...
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_advancedKi67_duration12_fixed"
                                    ] = True
                            elif cycle_code == 2:  # 'flow cytometry'
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_flowcyto_trate12"
                                ] = sval
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_flowcyto_duration12"
                                ] = invertf2s(
                                    sval
                                )  # invert
                                if rate.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_flowcyto_trate12_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_flowcyto_duration12_fixed"
                                    ] = True
                            elif cycle_code == 6:  # 'flow cytometry separated'
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_flowcytosep_trate12"
                                ] = sval
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_flowcytosep_duration12"
                                ] = invertf2s(
                                    sval
                                )  # invert
                                if rate.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_flowcytosep_trate12_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_flowcytosep_duration12_fixed"
                                    ] = True
                            elif cycle_code == 7:  # 'cycling quiescent'
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_quiescent_trate12"
                                ] = sval
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_quiescent_duration12"
                                ] = invertf2s(
                                    sval
                                )  # invert
                                if rate.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_quiescent_trate12_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_quiescent_duration12_fixed"
                                    ] = True
                        ...
                        if rate.attrib["end_index"] == "0":
                            if cycle_code == 0:  #'advanced Ki67'
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_advancedKi67_trate20"
                                ] = sval
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_advancedKi67_duration20"
                                ] = invertf2s(
                                    sval
                                )  # invert
                                if rate.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_advancedKi67_trate20_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_advancedKi67_duration20_fixed"
                                    ] = True
                            elif cycle_code == 2:  # 'flow cytometry'
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_flowcyto_trate20"
                                ] = sval
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_flowcytosep_duration20"
                                ] = invertf2s(
                                    sval
                                )  # invert
                                if rate.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_flowcyto_trate20_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "cycle_flowcyto_duration20_fixed"
                                    ] = True

                        elif rate.attrib["end_index"] == "3":
                            # if cycle_code == 6: # 'flow cytometry separated'
                            cell_def_tab.param_d[cell_def_name][
                                "cycle_flowcytosep_trate23"
                            ] = sval
                            cell_def_tab.param_d[cell_def_name][
                                "cycle_flowcytosep_duration23"
                            ] = invertf2s(
                                sval
                            )  # invert
                            if rate.attrib["fixed_duration"].lower() == "true":
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_flowcytosep_trate23_fixed"
                                ] = True
                                cell_def_tab.param_d[cell_def_name][
                                    "cycle_flowcytosep_duration23_fixed"
                                ] = True

                    elif (rate.attrib["start_index"] == "3") and (
                        rate.attrib["end_index"] == "0"
                    ):
                        # cell_def_tab.cycle_flowcytosep_trate30.setText(rate.text)
                        cell_def_tab.param_d[cell_def_name][
                            "cycle_flowcytosep_trate30"
                        ] = rate.text
                        cell_def_tab.param_d[cell_def_name][
                            "cycle_flowcytosep_duration30"
                        ] = invertf2s(
                            sval
                        )  # invert
                        if rate.attrib["fixed_duration"].lower() == "true":
                            cell_def_tab.param_d[cell_def_name][
                                "cycle_flowcytosep_trate30_fixed"
                            ] = True
                            cell_def_tab.param_d[cell_def_name][
                                "cycle_flowcytosep_duration30_fixed"
                            ] = True
                ...
                for pd in pd_uep:  # phase_duration
                    ...
                    if pd.attrib["index"] == "0":
                        if cycle_code == 0:  #'advanced Ki67'
                            ...
                            logging.debug(
                                f"------------ for live: sval for phase duration= {sval}"
                            )
                            cell_def_tab.param_d[cell_def_name][
                                "cycle_live_duration00"
                            ] = sval
            ...
            death_uep = uep.find(
                ".//cell_definition[" + str(idx) + "]//phenotype//death"
            )
            ...
            for death_model in death_uep.findall("model"):
                ...
                if "apoptosis" in death_model.attrib["name"].lower():
                    ...
                    cell_def_tab.param_d[cell_def_name]["apoptosis_death_rate"] = (
                        death_model.find("death_rate").text
                    )
                    ...
                    if pd_uep is not None:
                        ...
                        for pd in pd_uep:  # <duration index= ... >
                            ...
                            if pd.attrib["index"] == "0":
                                ...
                                if pd.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "apoptosis_phase0_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "apoptosis_trate01_fixed"
                                    ] = True
                                else:
                                    cell_def_tab.param_d[cell_def_name][
                                        "apoptosis_phase0_fixed"
                    ...
                    # 			<duration index="1" fixed_duration="true">86400.1</duration>
                        ...
                        if tr_uep is not None:
                            ...
                            for tr in tr_uep:  # <rate start_index= ... >
                                ...
                                logging.debug(
                                    f'start_index= {tr.attrib["start_index"]}'
                                )
                    ...
                    if params_uep is None:
                        ...
                        sys.exit(1)
                    cell_def_tab.param_d[cell_def_name]["apoptosis_unlysed_rate"] = (
                        params_uep.find("unlysed_fluid_change_rate").text
                    )
                    cell_def_tab.param_d[cell_def_name]["apoptosis_lysed_rate"] = (
                        params_uep.find("lysed_fluid_change_rate").text
                    )
                    cell_def_tab.param_d[cell_def_name]["apoptosis_cyto_rate"] = (
                        params_uep.find("cytoplasmic_biomass_change_rate").text
                    )
                    cell_def_tab.param_d[cell_def_name]["apoptosis_nuclear_rate"] = (
                        params_uep.find("nuclear_biomass_change_rate").text
                    )
                    cell_def_tab.param_d[cell_def_name]["apoptosis_calcif_rate"] = (
                        params_uep.find("calcification_rate").text
                    )
                    cell_def_tab.param_d[cell_def_name][
                        "apoptosis_rel_rupture_volume"
                    ] = params_uep.find("relative_rupture_volume").text
                ...
                # necrosis_params_path = necrosis_path + "parameters//"
                    ...
                    cell_def_tab.param_d[cell_def_name]["necrosis_death_rate"] = (
                        death_model.find("death_rate").text
                    )
                    ...
                    if pd_uep is not None:  # necrosis durations
                        ...
                        for pd in pd_uep:  # <duration index= ... >
                            ...
                            if pd.attrib["index"] == "0":
                                ...
                                if pd.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "necrosis_phase0_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "necrosis_trate01_fixed"
                                    ] = True
                                else:
                                    cell_def_tab.param_d[cell_def_name][
                                        "necrosis_phase0_fixed"
                                ...
                                if pd.attrib["fixed_duration"].lower() == "true":
                                    cell_def_tab.param_d[cell_def_name][
                                        "necrosis_phase1_fixed"
                                    ] = True
                                    cell_def_tab.param_d[cell_def_name][
                                        "necrosis_trate12_fixed"
                                    ] = True
                                else:
                                    cell_def_tab.param_d[cell_def_name][
                                        "necrosis_phase1_fixed"
                    ...
                    # ------------------------
                        ...
                        if tr_uep is not None:
                            ...
                            for tr in tr_uep:  # transition rate
                                ...
                                if tr.attrib["start_index"] == "0":
                                    rate = float(tr.text)
                                    if abs(rate) < 1.0e-6:  # rwh: necessary??
                                        rate = 1.0e-6
                                        # dval = 9.e99
                                    # else:
                                    # dval = rate * 60.0
                                    logging.debug(
                                        f" --- transition rate 01 (float) = {rate}"
                                    )
                                    ...
                                    if tr.attrib["fixed_duration"].lower() == "true":
                                        ...
                                        cell_def_tab.param_d[cell_def_name][
                                            "necrosis_phase0_duration_fixed"
                                        ] = True
                                    else:
                                        # cell_def_tab.param_d[cell_def_name]["necrosis_phase0_fixed"] = False
                                        cell_def_tab.param_d[cell_def_name][
                                    ...
                                    if abs(rate) < 1.0e-6:
                                        rate = 1.0e-6
                                    logging.debug(
                                        f" --- transition rate 12 (float) = {rate}"
                                    )
                                    cell_def_tab.param_d[cell_def_name][
                                        "necrosis_trate12"
                                    ] = str(rate)
                                    ...
                                    if tr.attrib["fixed_duration"].lower() == "true":
                                        ...
                                        cell_def_tab.param_d[cell_def_name][
                                            "necrosis_phase1_duration_fixed"
                                        ] = True
                                    else:
                                        cell_def_tab.param_d[cell_def_name][
                                            "necrosis_trate12_fixed"
                    ...
                    cell_def_tab.param_d[cell_def_name]["necrosis_unlysed_rate"] = (
                        params_uep.find("unlysed_fluid_change_rate").text
                    )
                    cell_def_tab.param_d[cell_def_name]["necrosis_lysed_rate"] = (
                        params_uep.find("lysed_fluid_change_rate").text
                    )
                    cell_def_tab.param_d[cell_def_name]["necrosis_cyto_rate"] = (
                        params_uep.find("cytoplasmic_biomass_change_rate").text
                    )
                    cell_def_tab.param_d[cell_def_name]["necrosis_nuclear_rate"] = (
                        params_uep.find("nuclear_biomass_change_rate").text
                    )
                    cell_def_tab.param_d[cell_def_name]["necrosis_calcif_rate"] = (
                        params_uep.find("calcification_rate").text
                    )
                    cell_def_tab.param_d[cell_def_name]["necrosis_rel_rupture_rate"] = (
                        params_uep.find("relative_rupture_volume").text
                    )
            ...
            cell_def_tab.param_d[cell_def_name]["volume_total"] = uep.find(
                volume_path + "total"
            ).text
            cell_def_tab.param_d[cell_def_name]["volume_fluid_fraction"] = uep.find(
                volume_path + "fluid_fraction"
            ).text
            cell_def_tab.param_d[cell_def_name]["volume_nuclear"] = uep.find(
                volume_path + "nuclear"
            ).text
            cell_def_tab.param_d[cell_def_name]["volume_fluid_change_rate"] = uep.find(
                volume_path + "fluid_change_rate"
            ).text
            cell_def_tab.param_d[cell_def_name]["volume_cytoplasmic_rate"] = uep.find(
                volume_path + "cytoplasmic_biomass_change_rate"
            ).text
            cell_def_tab.param_d[cell_def_name]["volume_nuclear_rate"] = uep.find(
                volume_path + "nuclear_biomass_change_rate"
            ).text
            cell_def_tab.param_d[cell_def_name]["volume_calcif_fraction"] = uep.find(
                volume_path + "calcified_fraction"
            ).text
            cell_def_tab.param_d[cell_def_name]["volume_calcif_rate"] = uep.find(
                volume_path + "calcification_rate"
            ).text
            cell_def_tab.param_d[cell_def_name]["volume_rel_rupture_vol"] = uep.find(
                volume_path + "relative_rupture_volume"
            ).text
            ...
            mechanics_path = (
                ".//cell_definition[" + str(idx) + "]//phenotype//mechanics//"
            )
            ...
            # ----------
            cell_adhesion_affinities_path = (
                ".//cell_definition["
                + str(idx)
                + "]//phenotype//mechanics//cell_adhesion_affinities"
            )
            logging.debug(
                f"---- cell_interactions_path= {cell_adhesion_affinities_path}"
            )
            ...
            if cap is None:
                ...
                logging.debug(
                    f"---- No cell_adhesion_affinities_path found. Setting to default."
                )
                ...
                cds_uep = cell_def_tab.xml_root.find(
                    ".//cell_definitions"
                )  # find unique entry point
                if cds_uep is None:
                ...
                for var in cap.findall("cell_adhesion_affinity"):
                    ...
                    logging.debug(
                        f'--> {cell_def_tab.param_d[cell_def_name]["cell_adhesion_affinity"]}'
                    )
            ...
            mechanics_options_path = (
                ".//cell_definition[" + str(idx) + "]//phenotype//mechanics//options//"
            )
            ...
            val = uep.find(
                mechanics_options_path + "set_relative_equilibrium_distance"
            ).text
            cell_def_tab.param_d[cell_def_name][
            ...
            val = uep.find(
                mechanics_options_path + "set_absolute_equilibrium_distance"
            ).text
            cell_def_tab.param_d[cell_def_name][
                "mechanics_absolute_equilibrium_distance"
            ] = val

            if (
                uep.find(mechanics_options_path + "set_relative_equilibrium_distance")
                .attrib["enabled"]
                .lower()
                == "true"
            ):
                cell_def_tab.param_d[cell_def_name][
                    "mechanics_relative_equilibrium_distance_enabled"
                ] = True
            else:
                cell_def_tab.param_d[cell_def_name][
                    "mechanics_relative_equilibrium_distance_enabled"
            ...
            if (
                uep.find(mechanics_options_path + "set_absolute_equilibrium_distance")
                .attrib["enabled"]
                .lower()
                == "true"
            ):
                cell_def_tab.param_d[cell_def_name][
                    "mechanics_absolute_equilibrium_distance_enabled"
                ] = True
            else:
                cell_def_tab.param_d[cell_def_name][
                    "mechanics_absolute_equilibrium_distance_enabled"
            ...
            motility_path = (
                ".//cell_definition[" + str(idx) + "]//phenotype//motility//"
            )
            ...
            motility_options_path = (
                ".//cell_definition[" + str(idx) + "]//phenotype//motility//options//"
            )
            ...
            if uep.find(motility_chemotaxis_path) is None:
                ...
                cell_def_tab.param_d[cell_def_name][
                    "motility_chemotaxis_substrate"
                ] = ""
                cell_def_tab.param_d[cell_def_name][
                    "motility_chemotaxis_towards"
                ] = True
            else:
                if (
                    uep.find(motility_chemotaxis_path + "enabled").text.lower()
                    == "true"
                ):
            ...
            #     </chemotactic_sensitivities>
            #   </advanced_chemotaxis>
            motility_advanced_chemotaxis_path = (
                motility_options_path + "advanced_chemotaxis//"
            )
            # motility_advanced_chemotaxis_path = motility_options_path + "advanced_chemotaxis"
            logging.debug(
                f"motility_advanced_chemotaxis_path= {motility_advanced_chemotaxis_path}"
            )
            ...
            logging.debug(
                f'chemotactic_sensitivity= {cell_def_tab.param_d[cell_def_name]["chemotactic_sensitivity"]}'
            )
            # sys.exit(-1)

            if (
                uep.find(motility_advanced_chemotaxis_path) is None
            ):  # advanced_chemotaxis not present in .xml
                logging.debug(
                    f"---- no motility_advanced_chemotaxis_path found. Setting to default values"
                )
                cell_def_tab.param_d[cell_def_name][
                    "motility_advanced_chemotaxis"
                ] = False
                cell_def_tab.param_d[cell_def_name][
                    "motility_advanced_chemotaxis_substrate"
                ] = ""
                # cell_def_tab.param_d[cell_def_name]['chemotactic_sensitivity'] = {}
                ...
                if (
                    uep.find(motility_advanced_chemotaxis_path + "enabled").text.lower()
                    == "true"
                ):
                    cell_def_tab.param_d[cell_def_name][
                        "motility_advanced_chemotaxis"
                ...
                if (
                    uep.find(
                        motility_advanced_chemotaxis_path + "normalize_each_gradient"
                    ).text.lower()
                    == "true"
                ):
                    cell_def_tab.param_d[cell_def_name][
                ...
                if uep.find(sensitivity_path) is None:
                    logging.debug(
                        f"---- chemotactic_sensitivities not found. Set to defaults (0). "
                    )
                    # sys.exit(-1)
                #       <chemotactic_sensitivity substrate="resource">0</chemotactic_sensitivity>
                else:
                    # cell_def_tab.param_d[cell_def_name]['chemotactic_sensitivity'] = {}
                    logging.debug(
                        f"---- found chemotactic_sensitivities: {uep.find(sensitivity_path)}"
                    )
                    for subelm in uep.find(sensitivity_path).findall(
                        ...
                        logging.debug(
                            f" chemotactic_sensitivity--> {subname} = {subval}"
                        )
                ...
                logging.debug(
                    f'{cell_def_tab.param_d[cell_def_name]["chemotactic_sensitivity"]}'
                )

                cell_def_tab.motility2_substrate_changed_cb(
                    0
                )  # update the sensitivity value in the widget
            ...
            secretion_path = (
                ".//cell_definition[" + str(idx) + "]//phenotype//secretion//"
            )
            logging.debug(f"secretion_path = {secretion_path}")
            secretion_sub1_path = (
                ".//cell_definition["
                + str(idx)
                + "]//phenotype//secretion//substrate[1]//"
            )

            uep_secretion = cell_def_tab.xml_root.find(
            ...
            cell_interactions_path = (
                ".//cell_definition[" + str(idx) + "]//phenotype//cell_interactions"
            )
            ...
            if cep is None:
                logging.debug(
                    f"---- no cell_interactions found. Setting to default values"
                )
            ...
            logging.debug(
                f' live_phagocytosis_rate= {cell_def_tab.param_d[cell_def_name]["live_phagocytosis_rate"]}'
            )
            logging.debug(
                f' attack_rate= {cell_def_tab.param_d[cell_def_name]["attack_rate"]}'
            )
            logging.debug(
                f' fusion_rate= {cell_def_tab.param_d[cell_def_name]["fusion_rate"]}'
            )
            logging.debug(f"------ done parsing cell_interactions:")

            # # --------- cell_transformations
            transformation_rates_path = (
                ".//cell_definition["
                + str(idx)
                + "]//phenotype//cell_transformations//transformation_rates"
            )
            logging.debug(
                f"---- transformation_rates_path = {transformation_rates_path}"
            )
            print(
            ...
            if trp is None:
                ...
                # sys.exit(-1)
                logging.debug(
                    f"---- No cell_transformations found. Setting to default values"
                )
                ...
                for var in cds_uep.findall("cell_definition"):
                    logging.debug(f' --> {var.attrib["name"]}')
                    print(
                        f"ugh!---- setting {cell_def_name}'s cell_transformations for {name} = default=0.0"
                    )
                    name = var.attrib["name"]
                    cell_def_tab.param_d[cell_def_name]["transformation_rate"][
                        name
                ...
                print(
                    f"---- found cell_transformations for {cell_def_name}, now loop thru them:"
                )
            ...
            logging.debug(
                f' transformation_rate= {cell_def_tab.param_d[cell_def_name]["transformation_rate"]}'
            )
            print(
                f'populate_tree_cell_defs.py: {cell_def_name}----> transformation_rate= {cell_def_tab.param_d[cell_def_name]["transformation_rate"]}'
            )
            ...
            if (
                uep_intracellular is not None and "type" in uep_intracellular.attrib
            ):  # also check for <intracellular />
                ...
                if uep_intracellular.attrib["type"] == "maboss":
                    ...
                    cell_def_tab.param_d[cell_def_name]["intracellular"][
                        ...
                        uep_intracellular.find("bnd_filename").text
                        if uep_intracellular.find("bnd_filename") is not None
                        else None
                    )
                    cell_def_tab.param_d[cell_def_name]["intracellular"][
                        "cfg_filename"
                    ] = (
                        uep_intracellular.find("cfg_filename").text
                        if uep_intracellular.find("cfg_filename") is not None
                        else None
                    )
                    ...
                    if uep_settings is not None:
                        ...
                        cell_def_tab.param_d[cell_def_name]["intracellular"][
                            ...
                            uep_settings.find("intracellular_dt").text
                            if uep_settings.find("intracellular_dt") is not None
                            else "12.0"
                        )
                        cell_def_tab.param_d[cell_def_name]["intracellular"][
                            "scaling"
                        ] = (
                            uep_settings.find("scaling").text
                            if uep_settings.find("scaling") is not None
                            else "1.0"
                        )
                        cell_def_tab.param_d[cell_def_name]["intracellular"][
                            "time_stochasticity"
                        ] = (
                            uep_settings.find("time_stochasticity").text
                            if uep_settings.find("time_stochasticity") is not None
                            else "0.0"
                        )
                        cell_def_tab.param_d[cell_def_name]["intracellular"][
                            "start_time"
                        ] = (
                            uep_settings.find("start_time").text
                            if uep_settings.find("start_time") is not None
                            else "0.0"
                        )
                        cell_def_tab.param_d[cell_def_name]["intracellular"][
                            "global_inheritance"
                        ] = (
                            uep_settings.find("inheritance").attrib["global"]
                            if uep_settings.find("inheritance") is not None
                            and uep_settings.find("inheritance").attrib["global"]
                            else "False"
                        )
                        cell_def_tab.param_d[cell_def_name]["intracellular"][
                            "node_inheritance"
                        ] = []
                        uep_intracellular_nodes_inheritance = uep_settings.find(
                            "inheritance"
                        )
                        if uep_intracellular_nodes_inheritance is not None:
                            for node_inheritance in uep_intracellular_nodes_inheritance:
                                cell_def_tab.param_d[cell_def_name]["intracellular"][
                                    ...
                                    {
                                        "node": node_inheritance.attrib[
                                            "intracellular_name"
                                        ],
                                        "flag": node_inheritance.text,
                                    }
                                )
                        ...
                        if uep_intracellular_mutants is not None:
                            for mutant in uep_intracellular_mutants:
                                cell_def_tab.param_d[cell_def_name]["intracellular"][
                                    ...
                                    {
                                        "node": mutant.attrib["intracellular_name"],
                                        "value": mutant.text,
                                    }
                                )
                        ...
                        if uep_intracellular_parameters is not None:
                            for parameter in uep_intracellular_parameters:
                                cell_def_tab.param_d[cell_def_name]["intracellular"][
                                    ...
                                    {
                                        "name": parameter.attrib["intracellular_name"],
                    ...
                    if uep_intracellular_iv is not None:
                        for initial_value in uep_intracellular_iv:
                            cell_def_tab.param_d[cell_def_name]["intracellular"][
                                ...
                                {
                                    "node": initial_value.attrib["intracellular_name"],
                    ...
                    if uep_intracellular_mappings is not None:
                        for mapping in uep_intracellular_mappings:
                            if mapping.tag == "input":
                                ...
                                if (
                                    input_settings is not None
                                    and input_settings.find("action") is not None
                                    and input_settings.find("threshold") is not None
                                ):
                                    ...
                                    cell_def_tab.param_d[cell_def_name][
                                    ...
                                    ]["inputs"].append(
                                        {
                                            ...
                                            "action": input_settings.find(
                                                "action"
                                            ).text,
                                            "threshold": input_settings.find(
                                                "threshold"
                                            ).text,
                                            "inact_threshold": (
                                                input_settings.find(
                                                ...
                                                if input_settings.find(
                                                    "inact_threshold"
                                                )
                                                is not None
                                                else input_settings.find(
                                                    "threshold"
                                                ).text
                                            ),
                                            "smoothing": (
                                                input_settings.find("smoothing").text
                                                if input_settings.find("smoothing")
                                                is not None
                                                else "0"
                                            ),
                                        }
                                ...
                                if (
                                    output_settings is not None
                                    and output_settings.find("action") is not None
                                    and output_settings.find("value") is not None
                                ):
                                    ...
                                    cell_def_tab.param_d[cell_def_name][
                                    ...
                                    ]["outputs"].append(
                                        {
                                            ...
                                            "action": output_settings.find(
                                                "action"
                                            ).text,
                                            "value": output_settings.find("value").text,
                                            "basal_value": (
                                                output_settings.find("base_value").text
                                                if output_settings.find("base_value")
                                                is not None
                                                else output_settings.find("value").text
                                            ),
                                            "smoothing": (
                                                output_settings.find("smoothing").text
                                                if output_settings.find("smoothing")
                                                is not None
                                                else "0"
                                            ),
                                        }
            ...
            uep_custom_data = cell_def_tab.xml_root.find(
                ".//cell_definitions//cell_definition[" + str(idx) + "]//custom_data"
            )
            ...
            if uep_custom_data:
                ...
                # cell_def_tab.param_d[cell_def_name]['custom_data'] = {}
                logging.debug(
                    f'--------------- populate_tree: (empty)custom_data = {cell_def_tab.param_d[cell_def_name]["custom_data"]}'
                )
                for var in uep_custom_data:
                    ...
                    if var.tag not in cell_def_tab.master_custom_var_d.keys():
                        ...
                        cell_def_tab.master_custom_var_d[var.tag] = [
                            cell_def_tab.custom_var_count,
                            "",
                            "",
                        ]  # [row#, units, desc]
                    ...
                    if (
                        "conserved" in var.attrib.keys()
                        and var.attrib["conserved"].lower() == "true"
                    ):
                    ...
                    if "units" in var.attrib.keys():
                        ...
                        if len(units_str) > len(
                            cell_def_tab.master_custom_var_d[var.tag][1]
                        ):
                            cell_def_tab.master_custom_var_d[var.tag][
                                1
                            ] = units_str  # hack: hard-coded index
                    ...
                    if "description" in var.attrib.keys():
                        ...
                        if len(desc_str) > len(
                            cell_def_tab.master_custom_var_d[var.tag][2]
                        ):
                            cell_def_tab.master_custom_var_d[var.tag][
                                2
                            ] = desc_str  # hack: hard-coded index
                    ...
                    cell_def_tab.param_d[cell_def_name]["custom_data"][var.tag] = [
                        val,
                        conserved_flag,
                    ]
                    # cell_def_tab.custom_var_d[var.tag] = [units_val, desc_str]
                    logging.debug(
                        f'populate: cell_def_name= {cell_def_name} --> custom_data: {cell_def_tab.param_d[cell_def_name]["custom_data"]}'
                    )
    ...
    cell_def_tab.tree.setCurrentItem(
        cell_def_tab.tree.topLevelItem(0)
    )  # select the top (0th) item
    cell_def_tab.tree_item_clicked_cb(
        cell_def_tab.tree.topLevelItem(0), 0
    )  # and have its params shown
    ...
    if uep_cell_rules:
        ...
        logging.debug(
            f"------- populate_tree_cell_defs.py: setting rules.csv folder = {rules_folder}"
        )
        logging.debug(
            f"------- populate_tree_cell_defs.py: setting rules.csv file = {rules_file}"
        )
