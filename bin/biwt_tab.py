...
from studio_classes import LegendWindow, QCheckBox_custom, QHLine, QVLine
...
class GoBackButton(QPushButton):
    def __init__(self, parent, biwt, pre_cb=None, post_cb=None):
        ...
        self.setStyleSheet(
            f"QPushButton {{background-color: lightgreen; color: black;}}"
        )
...
class ContinueButton(QPushButton):
    def __init__(
        ...
        cb,
        text="Continue \u2192",
        styleSheet="QPushButton {background-color: lightgreen; color: black;}",
    ):
        super().__init__(parent)
        self.setText(text)
...
class BioinformaticsWalkthroughWindow(QWidget):
    def __init__(self, biwt):
        super().__init__()
        self.setWindowTitle(
            f"Bioinformatics Import Walkthrough: Step {biwt.current_window_idx+1}"
        )
        self.biwt = biwt
        self.biwt.stale_futures = (
            True  # initializing a window means that any future windows are stale
        )
    ...
    def __init__(self, biwt):
        ...
        if self.biwt.column_line_edit.text() in tuple(self.biwt.data_columns.names):
            ...
            self.column_combobox.setCurrentIndex(
                self.column_combobox.findText(self.biwt.column_line_edit.text())
            )
        ...
        self.column_combobox.currentIndexChanged.connect(
            self.column_combobox_changed_cb
        )
...
class BioinformaticsWalkthroughWindow_EditCellTypes(BioinformaticsWalkthroughWindow):
    def __init__(self, biwt):
        ...
        self.biwt.cell_type_dict_on_edit = (
            {}
        )  # dictionary mapping original cell_type to intermediate/final cell type (depending on stage of walkthrough)
        ...
        self.merge_id = 0
        rename_button_style_sheet_template = (
            lambda x: f"""
            QPushButton {{
                color : black;
                font-weight : bold;
        ...
        self.rename_button_style_sheet["keep"] = rename_button_style_sheet_template(
            keep_color
        )
        self.rename_button_style_sheet["merge"] = rename_button_style_sheet_template(
            merge_color
        )
        self.rename_button_style_sheet["delete"] = rename_button_style_sheet_template(
            delete_color
        )
        row_height = 30
        ...
        for cell_type in self.biwt.cell_types_list_original:
            hbox = QHBoxLayout()
            self.checkbox_dict_edit[cell_type] = QCheckBox(
                cell_type, styleSheet=self.checkbox_style["keep"]
            )
            ...
            self.keep_button[cell_type] = QPushButton(
                "Keep", enabled=False, objectName=cell_type
            )
            self.keep_button[cell_type].setStyleSheet(
                rename_button_style_sheet_template(keep_color)
            )
        ...
        self.cell_type_edit_scroll_area.setVerticalScrollBarPolicy(
        ...
        self.merge_button = QPushButton(
            "Merge",
            enabled=False,
            styleSheet=rename_button_style_sheet_template(merge_color),
        )
        ...
        self.delete_button = QPushButton(
            "Delete",
            enabled=False,
            styleSheet=rename_button_style_sheet_template(delete_color),
        )
        ...
        for key_substring in ["umap", "tsne", "pca", "spatial"]:
            if self.try_to_plot_dim_red(key_substring=key_substring):
                ...
                self.obsm_keys_combobox.currentIndexChanged.connect(
                    self.obsm_keys_combobox_changed_cb
                )
    ...
    def try_to_plot_dim_red(self, key_substring="umap"):
        found, k, _ = self.biwt.search_vis_arrays_for(
            key_substring
        )  # (was this found?, the key in adata.obsm, the value of adata.obsm[k])
        if not found:
            return False
        self.plot_dim_red(k)
        return True
    ...
    def plot_dim_red(self, k):
        ...
        self.scatter = self.dim_red_ax.scatter(
            v[indices, 0], v[indices, 1], self.marker_size, c=temp.codes[indices]
        )  # 0.18844459036110225 = 5/sqrt(704) where I found 5 to be a good size when working with 704 points
        scatter_objects, _ = self.scatter.legend_elements()
        self.legend_window = LegendWindow(
            self,
            legend_artists=scatter_objects,
            legend_labels=temp.categories,
    ...
    def set_cell_type_to_keep(self, cell_type, check_merge_gp=True):
        ...
        if check_merge_gp and (
            "\u21d2 Merge Gp. #" in self.checkbox_dict_edit[cell_type].text()
        ):
            # check for merge group that is no longer merging
            gp_preimage = [
                ctn
                for ctn, new_name in self.biwt.cell_type_dict_on_edit.items()
                if (
                    ctn != cell_type
                    and new_name == self.biwt.cell_type_dict_on_edit[cell_type]
                )
            ]  # get cell types that map into this merge group
            if len(gp_preimage) == 1:  # then delete this merge gp:
                self.set_cell_type_to_keep(gp_preimage[0], check_merge_gp=False)
            elif (
                cell_type == self.biwt.cell_type_dict_on_edit[cell_type]
            ):  # make sure this current cell type did not set the merge group name
                first_name = None
    ...
    def delete_cb(self):
        ...
        for cell_type in self.checkbox_dict_edit.keys():
            if self.checkbox_dict_edit[cell_type].isChecked():
                ...
                self.checkbox_dict_edit[cell_type].setStyleSheet(
                    self.checkbox_style["delete"]
                )
    ...
    def merge_cb(self):
        ...
        for cell_type in self.checkbox_dict_edit.keys():
            if self.checkbox_dict_edit[cell_type].isChecked():
                ...
                self.checkbox_dict_edit[cell_type].setEnabled(False)
                self.checkbox_dict_edit[cell_type].setStyleSheet(
                    self.checkbox_style["merge"]
                )
                self.checkbox_dict_edit[cell_type].setText(
                    f"{cell_type} \u21d2 Merge Gp. #{self.merge_id}"
                )
    ...
    def marker_size_changed_cb(self, text):
        ...
        self.scatter.set_sizes(
            self.marker_size + np.zeros_like(self.scatter.get_sizes())
        )
        # self.scatter.draw()
        self.dim_red_canvas.update()
        self.dim_red_canvas.draw()
...
class BioinformaticsWalkthroughWindow_RenameCellTypes(BioinformaticsWalkthroughWindow):
    def __init__(self, biwt):
        ...
        for intermediate_type in self.biwt.intermediate_types:
            hbox = QHBoxLayout()
            label_text = ", ".join(
                self.biwt.intermediate_type_pre_image[intermediate_type]
            )
            label_text += " \u21d2 "
            labels[intermediate_type] = QLabel(label_text)
            self.new_name_line_edit[intermediate_type] = QLineEdit()
            self.new_name_line_edit[intermediate_type].setText(
                self.biwt.intermediate_type_pre_image[intermediate_type][0]
            )
            self.new_name_line_edit[intermediate_type].textChanged.connect(
                self.intermediate_type_renamed
            )
    ...
    def process_window(self):
        ...
        for intermediate_type in self.biwt.intermediate_types:
            self.biwt.cell_types_list_final.append(
                self.new_name_line_edit[intermediate_type].text()
            )
            for cell_type in self.biwt.intermediate_type_pre_image[intermediate_type]:
                self.biwt.cell_type_dict_on_rename[cell_type] = self.new_name_line_edit[
                    intermediate_type
                ].text()
...
class BioinformaticsWalkthroughWindow_CellCounts(BioinformaticsWalkthroughWindow):
    def __init__(self, biwt):
        ...
        vbox.addWidget(
            QLabel(
                "Set how many of each cell type to place. You may use the counts found in your data, or use proportions or confluence to scale these counts.\nYou may also set the counts manually; these values will track the others, allowing you to adjust from those baselines."
            )
        )
        ...
        self.counts_button_group.addButton(
            self.use_counts_as_is_radio_button, counts_button_group_next_id
        )
        ...
        self.counts_button_group.addButton(
            self.use_props_radio_button, counts_button_group_next_id
        )
        counts_button_group_next_id += 1
        ...
        self.counts_button_group.addButton(
            self.use_confluence_radio_button, counts_button_group_next_id
        )
        counts_button_group_next_id += 1
        ...
        self.counts_button_group.addButton(
            self.use_manual_radio_button, counts_button_group_next_id
        )
        counts_button_group_next_id += 1
        ...
        self.cell_type_props = [
            self.biwt.cell_counts[cell_type] / len(self.biwt.cell_types_final)
        ...
        for idx, cell_type in enumerate(self.biwt.cell_types_list_final):
            ...
            self.type_confluence[cell_type].setStyleSheet(
                self.biwt.qlineedit_style_sheet
            )
            self.type_confluence[cell_type].setValidator(
                QtGui.QDoubleValidator(bottom=0)
            )
            self.type_confluence[cell_type].setObjectName(str(idx))
            self.type_confluence[cell_type].textChanged.connect(
                self.confluence_box_changed_cb
            )
        ...
        self.total_conf.setText(
            "100"
        )  # this triggers the cb, which sets the manual column (see below) so force a toggle next
    ...
    def setup_confluence_info(self):
        ...
        volume_env = (
            float(self.biwt.config_tab.xmax.text())
            - float(self.biwt.config_tab.xmin.text())
        ) * (
            float(self.biwt.config_tab.ymax.text())
            - float(self.biwt.config_tab.ymin.text())
        )
        self.prop_total_area_one = {}
        self.prop_dot_ratios = 0
        for idx, cell_type in enumerate(self.biwt.cell_types_list_final):
            self.prop_total_area_one[cell_type] = (
                ((9 * np.pi * self.biwt.cell_volume[cell_type] ** 2) / 16) ** (1.0 / 3)
            ) / volume_env
            self.prop_dot_ratios += (
                self.cell_type_props[idx] * self.prop_total_area_one[cell_type]
            )
    ...
    def prop_box_changed_cb(self, text):
        ...
        for idx, cell_type in enumerate(self.biwt.cell_types_list_final):
            if current_name == str(idx):
                continue
            self.type_prop[cell_type].setText(
                str(round(mult * self.cell_type_props[idx]))
            )
            self.type_manual[cell_type].setText(
                str(round(mult * self.cell_type_props[idx]))
            )
        self.prop_box_callback_paused = False
    ...
    def confluence_box_changed_cb(self, text):
        ...
        if current_name == "total_conf":
            ...
            mult = current_conf
            mult /= self.prop_total_area_one[
                self.biwt.cell_types_list_final[current_idx]
            ]
        ...
        for idx, cell_type in enumerate(self.biwt.cell_types_list_final):
            if current_name == str(idx):
                ...
                continue
            new_conf = (
                mult * self.cell_type_props[idx] * self.prop_total_area_one[cell_type]
            )
            total_conf += new_conf
    ...
    def convert_confluence_to_counts(self):
        ...
        for cell_type in self.biwt.cell_types_list_final:
            n = round(
                0.01
                * float(self.type_confluence[cell_type].text())
                / self.prop_total_area_one[cell_type]
            )
            counts[cell_type] = n
            total += n
    ...
    def process_window(self):
        ...
        if id == 1:  # use props found in data file
            ...
            for cell_type in self.biwt.cell_types_list_final:
                self.biwt.cell_counts[cell_type] = int(
                    self.type_manual[cell_type].text()
                )
...
class BioinformaticsWalkthroughWindow_PositionsWindow(BioinformaticsWalkthroughWindow):
    def __init__(self, biwt):
        ...
        self.ics_plot_area = BioinformaticsWalkthroughPlotWindow(
            self, self.biwt, self.biwt.config_tab
        )
        ...
        self.continue_to_write_button = ContinueButton(
            self, self.process_window, styleSheet=self.biwt.qpushbutton_style_sheet
        )
    ...
    def create_cell_type_scroll_area(self):
        ...
        self.cell_type_button_group.buttonClicked.connect(
            self.cell_type_button_group_cb
        )
        self.checkbox_dict = create_checkboxes_for_cell_types(
            vbox_mid_checkboxes, self.biwt.cell_types_list_final
        )
        self.undo_button = {}
        ...
        undo_qpushbutton_style_sheet = """
            ...
            """
        for cell_type in self.biwt.cell_types_list_final:
            self.undo_button[cell_type] = QPushButton(
                "Undo",
                enabled=False,
                styleSheet=undo_qpushbutton_style_sheet,
                objectName=cell_type,
            )
        ...
        self.select_all_button = QPushButton(
            "Select remaining", styleSheet=self.biwt.qpushbutton_style_sheet
        )
        self.select_all_button.clicked.connect(self.select_all_button_cb)

        self.deselect_all_button = QPushButton(
            "Unselect all", styleSheet=self.biwt.qpushbutton_style_sheet
        )
        ...
        self.undo_all_button = QPushButton(
            "Undo All", enabled=False, styleSheet=undo_qpushbutton_style_sheet
        )
    ...
    def create_pos_scroll_area(self):
        ...
        vbox = QVBoxLayout()
        full_rectangle_button = QPushButton(
            icon=QIcon(sys.path[0] + "/icon/scatter_square.svg"),
            iconSize=icon_size,
            checkable=True,
            checked=(not self.biwt.use_spatial_data),
        )
        ...
        vbox = QVBoxLayout()
        partial_rectangle_button = QPushButton(
            icon=QIcon(sys.path[0] + "/icon/rectangle.svg"),
            iconSize=icon_size,
            checkable=True,
            checked=False,
        )
        ...
        vbox = QVBoxLayout()
        disc_button = QPushButton(
            icon=QIcon(sys.path[0] + "/icon/disc.svg"),
            iconSize=icon_size,
            checkable=True,
            checked=False,
        )
        ...
        vbox = QVBoxLayout()
        annulus_button = QPushButton(
            icon=QIcon(sys.path[0] + "/icon/annulus.svg"),
            iconSize=icon_size,
            checkable=True,
            checked=False,
        )
        ...
        vbox = QVBoxLayout()
        wedge_button = QPushButton(
            icon=QIcon(sys.path[0] + "/icon/wedge.svg"),
            iconSize=icon_size,
            checkable=True,
            checked=False,
        )
        ...
        vbox = QVBoxLayout()
        rainbow_button = QPushButton(
            icon=QIcon(sys.path[0] + "/icon/rainbow.svg"),
            enabled=False,
            iconSize=icon_size,
            checkable=True,
            checked=False,
        ...
        if self.biwt.use_spatial_data:
            ...
            vbox = QVBoxLayout()
            spatial_button = QPushButton(
                icon=QIcon(sys.path[0] + "/icon/spatial.png"),
                enabled=True,
                checkable=True,
                iconSize=icon_size,
            )
    ...
    def cell_pos_button_group_cb(self):
        ...
        if self.biwt.use_spatial_data:
            self.ics_plot_area.num_box.setEnabled(
                self.cell_pos_button_group.checkedId() == 6
            )  # only enable for spatial plotter
    ...
    def replot_all_cells_after_undo(self):
        ...
        for cell_type in self.biwt.csv_array.keys():
            ...
            if self.ics_plot_area.plot_is_2d:
                sz = np.sqrt(
                    self.ics_plot_area.cell_type_micron2_area_dict[cell_type] / np.pi
                )
                self.ics_plot_area.circles(
                    self.biwt.csv_array[cell_type],
                    s=sz,
                    color=self.ics_plot_area.color_by_celltype[cell_type],
                    edgecolor="black",
                    linewidth=0.5,
                    alpha=self.ics_plot_area.alpha_value,
                )
                legend_patch = Patch(
                    facecolor=self.ics_plot_area.color_by_celltype[cell_type],
                    edgecolor="black",
                    linewidth=0.5,
                )
                ...
                collections = self.ics_plot_area.ax0.scatter(
                    self.biwt.csv_array[cell_type][:, 0],
                    self.biwt.csv_array[cell_type][:, 1],
                    self.biwt.csv_array[cell_type][:, 2],
                    s=8.0,
                    color=self.ics_plot_area.color_by_celltype[cell_type],
                    alpha=self.ics_plot_area.alpha_value,
                )
...
class BioinformaticsWalkthroughWindow_WritePositions(BioinformaticsWalkthroughWindow):
    def __init__(self, biwt):
        ...
        label = QLabel(
            "Confirm output file and then overwrite or append with these cell positions."
        )
        vbox.addWidget(label)
        ...
        self.finish_write_button.setStyleSheet(
            "QPushButton {background-color : yellow; font-weight : bold;}"
        )
        self.finish_write_button.clicked.connect(self.finish_write_button_cb)

        self.finish_append_button = QPushButton("Append")
        self.finish_append_button.setStyleSheet(
            "QPushButton {background-color : yellow; font-weight : bold;}"
        )
    ...
    def finish_append_button_cb(self):
        ...
        with open(self.full_fname, "r") as f:
            ...
            if "x,y,z,type" not in first_line:
                print(
                    f"{self.full_fname} is not properly formatted for appending.\nIt needs to start with 'x,y,z,type,...'"
                )
                return
        self.add_cell_positions_to_file()
    ...
    def check_for_new_celldefs(self):
        print(
            "BioinformaticsWalkthroughPlotWindow: Checking for new cell definitions..."
        )
        for cell_type in self.biwt.cell_types_list_final:
            if cell_type in self.biwt.celldef_tab.celltypes_list:
                print(
                    f"BioinformaticsWalkthroughPlotWindow: {cell_type} found in current list of cell types. Not appending {cell_type}..."
                )
            else:
                print(
                    f"BioinformaticsWalkthroughPlotWindow: {cell_type} not found in current list of cell types. Appending {cell_type}..."
                )
...
class BioinformaticsWalkthroughPlotWindow(QWidget):
    def __init__(self, positions_window, biwt, config_tab):
        ...
        if len(self.biwt.cell_types_list_final) <= len(value):
            ...
            value = [
                value[idx % len(value)]
                for idx in range(len(self.biwt.cell_types_list_final))
            ]
        ...
        self.plot_is_2d = (
            self.plot_zmax - self.plot_zmin <= self.plot_zdel
        )  # if the domain height is larger than the voxel height, then we have a 3d simulation
    ...
    def create_spot_num_box(self):
        ...
        self.num_box.setStyleSheet(
            "QSpinBox {color : black; background-color : white} QSpinBox:disabled {background-color : lightgray;}"
        )
        hbox = QHBoxLayout()
        hbox.addStretch(1)
        hbox.addWidget(label)
    ...
    def default_spatial_pars(self):
        ...
        spatial_factors = [
            self.plot_dx / (xR - xL),
            self.plot_dy / (yR - yL),
        ]  # factors for scaling each dimension to an interval of length 1
        ...
        print(
            f"spatial_factors = {spatial_factors}, spatial_factor = {spatial_factor}\n\n\n"
        )
        self.spatial_base_coords = np.hstack(
            (
                self.spatial_base_coords,
                (self.biwt.spatial_data_final[:, 2].reshape((-1, 1)) - zL) / (zR - zL),
            )
        )
    ...
    def sync_par_area(self):
        ...
        if id == 0:
            ...
            self.mpl_cid.append(
                self.canvas.mpl_connect("button_release_event", self.mouse_released_cb)
            )  # only appending to history on release; the motion doesn't add to history because it holds focus and so editingFinished signal not emitted while canvas holds focus
            if id == 1:
                ...
                self.mpl_cid.append(
                    self.canvas.mpl_connect(
                        "button_press_event", self.rectangle_mouse_press
                    )
                )
                self.mpl_cid.append(
                    self.canvas.mpl_connect(
                        "motion_notify_event", self.rectangle_mouse_motion
                    )
                )
                ...
                self.mpl_cid.append(
                    self.canvas.mpl_connect("button_press_event", self.disc_mouse_press)
                )
                self.mpl_cid.append(
                    self.canvas.mpl_connect(
                        "motion_notify_event", self.disc_mouse_motion
                    )
                )
                ...
                self.mpl_cid.append(
                    self.canvas.mpl_connect(
                        "button_press_event", self.annulus_mouse_press
                    )
                )
                self.mpl_cid.append(
                    self.canvas.mpl_connect(
                        "motion_notify_event", self.annulus_mouse_motion
                    )
                )
                ...
                self.mpl_cid.append(
                    self.canvas.mpl_connect(
                        "button_press_event", self.wedge_mouse_press
                    )
                )
                self.mpl_cid.append(
                    self.canvas.mpl_connect(
                        "motion_notify_event", self.wedge_mouse_motion
                    )
                )
    ...
    def everywhere_plotter(self):
        ...
        self.plot_cells_button.setEnabled(
            self.pw.is_any_cell_type_button_group_checked()
        )
    ...
    def default_x0y0(self):
        return (
            0.5 * (self.plot_xmin + self.plot_xmax),
            0.5 * (self.plot_ymin + self.plot_ymax),
        )
    ...
    def disc_mouse_press(self, event):
        self.mouse_pressed = self.standard_mouse_press(
            event, lambda e, x0, y0: self.set_radius_helper(e, x0, y0, 2)
        )
    ...
    def wedge_mouse_press(self, event):
        ...
        if modifiers == QtCore.Qt.AltModifier:  # option-click
            ...
            self.assign_par(
                theta, 4
            )  # in this case, fix theta1 and then let theta2 vary
            idx = 5
        elif modifiers == (
    ...
    def rectangle_plotter(self):
        if self.plot_is_2d:
            ...
            if self.preview_patch is None:
                self.preview_patch = self.ax0.add_patch(
                    Rectangle((x0, y0), width, height, alpha=0.2)
                )
            else:
        ...
        bval = (
            (x0 < self.plot_xmax)
            and (x0 + width > self.plot_xmin)
            and (y0 < self.plot_ymax)
            and (y0 + height > self.plot_ymin)
        )  # make sure the rectangle intersects the domain with positive area
        if (
            not self.plot_is_2d
        ):  # FIX (this will need a fix once we have 3d plotting fully implemented)
            z0 = self.plot_zmin
            depth = self.plot_zmax - self.plot_zmin
            bval = bval and (z0 < self.plot_zmax) and (z0 + depth > self.plot_zmin)

        self.plot_cells_button.setEnabled(
            bval and self.pw.is_any_cell_type_button_group_checked()
        )
    ...
    def disc_plotter(self):
        ...
        bval = (
            r2 < r * r
        )  # make sure the distance from center of Circle to domain is less than radius of circle
        if (
            not self.plot_is_2d
        ):  # FIX (this will need a fix once we have 3d plotting fully implemented)
            z0 = self.plot_zmin
    ...
    def annulus_plotter(self):
        if self.plot_is_2d:
            ...
            if r1 == 0 or (r1 < r0):
                if (
                    self.preview_patch
                ):  # probably a way to impose this using validators, but that would require dynamically updating the validators...
                    self.preview_patch.remove()
                    self.canvas.update()
                    self.canvas.draw()
                    self.preview_patch = None
            ...
            try:
                ...
                print(
                    "\tBIWT WARNING: You likely can use an update to matplotlib to fix a bug in their Annulus plots.\n\tWe'll take care of it for now."
                )
    ...
    def wedge_in_domain(self, x0, y0, r0, r1, th1, th2, dx, dy, r2):
        ...
        if r2 == 0:  # then (x0,y0) is in domain
            ...
            starting_theta_step = 1 + (
                th1 // 90
            )  # first 90 deg angle that could go to side of domain for shortest distance purposes
            end_theta_step = 1 + (
                th2 // 90
            )  # first 90 deg angle that could go to side of domain for shortest distance purposes
    ...
    def wedge_in_domain_center_out(self, x0, y0, r0, r1, th1, th2, dx, dy):
        ...
        bounding_th_R = np.arctan2(
            yR, xL
        )  # top-left corner is always the upper (Right) theta bound in this reference frame
        bounding_d_R2 = xL**2 + yR**2 + np.zeros((2, 1))
        inner_th_1 = np.arctan2(
            yR, xR
        )  # top-right is always interior in this frame of reference
        if dy == 0:
            inner_1_d2 = np.array(
                [(xL / np.cos(inner_th_1)) ** 2, xR**2 + yR**2]
            ).reshape((2, 1))
            bounding_th_L = np.arctan2(yL, xL)
            bounding_d_L2 = xL**2 + yL**2 + np.zeros((2, 1))
            inner_th_2 = np.arctan2(yL, xR)
            inner_2_d2 = np.array(
                [(xL / np.cos(inner_th_2)) ** 2, xR**2 + yL**2]
            ).reshape((2, 1))
            th_right_d2 = np.array([xL**2, xR**2]).reshape((2, 1))
            TH = np.array([bounding_th_L, inner_th_2, 0, inner_th_1, bounding_th_R])
            D = np.concatenate(
                (bounding_d_L2, inner_2_d2, th_right_d2, inner_1_d2, bounding_d_R2),
                axis=1,
            )
        else:
            ...
            inner_1_d2 = np.array(
                [(np.max([temp_dx, temp_dy])) ** 2, xR**2 + yR**2]
            ).reshape((2, 1))
            ...
            inner_2_d2 = np.array(
                [xL**2 + yL**2, (np.min([temp_dx, temp_dy])) ** 2]
            ).reshape((2, 1))
            ...
            D = np.concatenate(
                (bounding_d_L2, inner_1_d2, inner_2_d2, bounding_d_R2), axis=1
            )

        th1_inbounds = th1_rad > bounding_th_L and th1_rad < bounding_th_R
        if th1_inbounds and (th1_rad not in TH):
            TH, D = compute_theta_intersection_distances(
                TH, D, th1_rad, x0, y0, xL, xR, yL, yR, dy
            )
        th2_inbounds = th2_rad > bounding_th_L and th2_rad < bounding_th_R
        th2_inbounds = th2_inbounds or (
            (th2_rad - 2 * np.pi) > bounding_th_L
            and (th2_rad - 2 * np.pi) < bounding_th_R
        )  # it's possible that th2 being on [th1,th1+360] might not lie between these theta value, but intersect nonetheless
        if th2_inbounds and (th2_rad not in TH):
            TH, D = compute_theta_intersection_distances(
                TH, D, th2_rad, x0, y0, xL, xR, yL, yR, dy
            )
        ...
        A = np.concatenate(
            (TH_rel.reshape(1, len(TH_rel)), TH.reshape(1, len(TH)), D), axis=0
        )
        ord = np.argsort(A[0])
        A = A[:, ord]
        ...
        for idx, abs_th in enumerate(A[1]):
            ...
            if d_old2 is not None:
                if (d_old2[0] >= r12 and d_new2[1] <= r02) or (
                    d_old2[1] <= r02 and d_new2[0] >= r12
                ):
    ...
    def spatial_plotter(self):
        ...
        if self.plot_is_2d:
            ...
            if self.preview_patch is None:
                ...
                offset = self.initial_offsets + (
                    self.spatial_base_coords
                    * [width - self.initial_width, height - self.initial_height]
                    + [x0 - self.initial_x0, y0 - self.initial_y0]
                )
    ...
    def create_figure(self):
        ...
        self.cell_type_micron2_area_dict = {
            cell_type: (((9 * np.pi * V**2) / 16) ** (1.0 / 3))
            for cell_type, V in self.biwt.cell_volume.items()
        }
        if self.biwt.use_spatial_data or (not self.plot_is_2d):
            dx, dy = self.ax0.transData.transform(
                (1, 1)
            ) - self.ax0.transData.transform(
                (0, 0)
            )  # # pixels/ micron
            ...
            self.cell_type_pt_area_dict = {
                cell_type: fudge_factor * area_scale_factor * A
                for cell_type, A in self.cell_type_micron2_area_dict.items()
            }
            self.single_scatter_sizes = np.array(
                [self.cell_type_pt_area_dict[ctn] for ctn in self.biwt.cell_types_final]
            )
            if self.biwt.use_spatial_data:
    ...
    def update_legend_window(self):
        ...
        is_hidden = (self.legend_window is None) or (
            self.legend_window.isHidden()
        )  # doesn't exist yet or is hidden, so don't display after update
        if self.legend_window is not None:
            self.legend_window.close()
        self.legend_window = LegendWindow(
            self,
    ...
    def canvas_key_press(self, event):
        ...
        if id == 0 or (event.key() != QtCore.Qt.Key_Z):
            ...
            self.patch_history_idx[id] += 1
        self.load_previous_patch_pars(
            self.patch_history[id][self.patch_history_idx[id]]
        )
        ...
        if self.pw.cell_pos_button_group.checkedId() == self.pw.spatial_plotter_id:
            ...
            for cell_type in self.pw.checkbox_dict.keys():
                if self.pw.checkbox_dict[cell_type].isChecked():
                    idx_cell_type = [
                        ctn == cell_type for ctn in self.biwt.cell_types_final
                    ]
                    cell_radius = np.sqrt(
                        self.cell_type_micron2_area_dict[cell_type] / np.pi
                    )
                    cell_coords = np.hstack(
                        (
                            self.spatial_base_coords[idx_cell_type, 0:2]
                            * [width, height]
                            + [x0, y0],
                            np.zeros((sum(idx_cell_type), 1)),
                        )
                    )
                    idx_inbounds = [
                        (
                            cc[0] >= self.plot_xmin
                            and cc[0] <= self.plot_xmax
                            and cc[1] >= self.plot_ymin
                            and cc[1] <= self.plot_ymax
                        )
                        for cc in cell_coords
                    ]
                    cell_coords = cell_coords[idx_inbounds, :]
                    if n_per_spot == 1:
                        self.biwt.csv_array[cell_type] = np.vstack(
                            (self.biwt.csv_array[cell_type], cell_coords)
                        )
                        if self.plot_is_2d:
                            self.circles(
                                cell_coords,
                                s=cell_radius,
                                color=self.color_by_celltype[cell_type],
                                edgecolor="black",
                                linewidth=0.5,
                                alpha=self.alpha_value,
                            )
                            legend_patch = Patch(
                                facecolor=self.color_by_celltype[cell_type],
                                edgecolor="black",
                                linewidth=0.5,
                            )
                            self.legend_artists.append(legend_patch)
                            ...
                            cell_coords[:, 2] = (
                                self.spatial_base_coords[idx_cell_type, 2] * depth + z0
                            )
                            collection = self.ax0.scatter(
                                cell_coords[:, 0],
                                cell_coords[:, 1],
                                cell_coords[:, 2],
                                s=8.0,
                        ...
                        self.biwt.csv_array[cell_type] = np.vstack(
                            (self.biwt.csv_array[cell_type], all_new)
                        )
                        if self.plot_is_2d:
                            self.circles(
                                all_new,
    ...
    def max_dist_to_domain(self, x0, y0):
        ...
        dx = (
            xL - x0 if (2 * x0 > xL + xR) else x0 - xR
        )  # distance to furtherst vertical edge
        dy = (
            yL - y0 if (2 * y0 > yL + yR) else y0 - yR
        )  # distance to furtherst horizontal edge
        return np.sqrt(dx * dx + dy * dy)

    def constrain_rectangle_to_domain(self):
        if self.preview_constrained_to_axes:
            return
        x0, y0, width, height = self.constrain_corners(
    ...
    def plot_cell_pos_single_2d(self, cell_type):
        ...
        if type(self.preview_patch) is Rectangle:
            ...
            x0, y0 = self.preview_patch.get_center()
            r1 = self.preview_patch.get_radii()[
                0
            ]  # annulus is technically an ellipse, get_radii returns (semi-major,semi-minor) axis lengths, since I'm using circles, these will be the same
            width = self.preview_patch.get_width()
            r0 = r1 - width
            if not self.preview_constrained_to_axes:
            ...
            if not self.preview_constrained_to_axes:
                ...
                if th2 == th1:
                    ...
                    th2 -= 360 * (
                        (th2 - th1) // 360
                    )  # I promise this works if dth=th2-th1 < 0, 0<dth<360, and dth>360.
                r0 = np.max([r0, np.sqrt(self.get_distance2_to_domain(x0, y0)[0])])
                self.preview_patch.set(
                    center=(x0, y0), radius=r1, theta1=th1, theta2=th2, width=r1 - r0
                )
            ...
            print("unknown patch")
        self.biwt.csv_array[cell_type] = np.append(
            self.biwt.csv_array[cell_type], self.new_pos, axis=0
        )

        self.circles(
            self.new_pos,
            s=(0.75 * self.biwt.cell_volume[cell_type] / np.pi) ** (1 / 3),
            color=self.color_by_celltype[cell_type],
            edgecolor="black",
            linewidth=0.5,
            alpha=self.alpha_value,
    ...
    def wedge_sample(self, N, x0, y0, r1, r0=0.0, th_lim=(0, 2 * np.pi)):
        ...
        while i_start < N:
            if r0 == 0:
                ...
                d = np.sqrt(
                    r0 * r0 + (r1 * r1 - r0 * r0) * np.random.uniform(size=N - i_start)
                )
            th = th_lim[0] + (th_lim[1] - th_lim[0]) * np.random.uniform(
                size=N - i_start
            )
            x = x0 + d * np.cos(th)
            y = y0 + d * np.sin(th)
            xy = np.array(
                [
                    ...
                    for a, b in zip(x, y)
                    if a >= self.plot_xmin
                    and a <= self.plot_xmax
                    and b >= self.plot_ymin
                    and b <= self.plot_ymax
                ]
            )
            if len(xy) == 0:
                continue
...
class BioinformaticsWalkthrough(QWidget):
    def __init__(self, config_tab, celldef_tab, ics_tab):
        ...
        hbox.addStretch()
        title_label = QLabel(
            '<p style="font-size:32px; text-decoration:underline;"><b>B</b>io<b>I</b>nformatics <b>W</b>alk<b>T</b>hrough (BIWT)</p>'
        )
        ...
        vbox.addWidget(
            QLabel(
                "Importing",
                styleSheet="QLabel {background-color : orange;}",
        ...
        label = QLabel(
            "Column name with cell type (prompt will follow if left blank): "
        )
        hbox.addWidget(label)
        ...
        label = QLabel(
            "Currently supported (file format, data type) pairs: (h5ad, anndata) and (csv, NA)"
        )
        vbox.addWidget(label)
    ...
    def open_next_window(self, window_class, layout=None, show=True):
        if self.window is not None:
            ...
            if self.stale_futures:
                ...
                if (
                    type(self.window)
                    is not BioinformaticsWalkthroughWindow_WarningWindow
                ):
                ...
                self.stale_futures = (
                    self.current_window_idx == len(self.previous_windows) - 1
                )  # if it's now the last one, mark it as stale
                # if self.stale_futures:
    ...
    def go_back_to_prev_window(self):
        ...
        if (
            self.stale_futures
            and self.current_window_idx < len(self.previous_windows) - 1
        ):
            print(
                f"\tFutures are stale. Deleting from {self.current_window_idx+2} to {len(self.previous_windows)}"
            )
            del self.previous_windows[self.current_window_idx + 1 :]
        elif not self.stale_futures:
            print(
                f"\tFutures are not stale. Keeping windows {self.current_window_idx+1} to {len(self.previous_windows)}"
            )
    ...
    def start_walkthrough(self):
        ...
        self.current_window_idx = (
            0  # reset the window index as this is an entry point for the walkthrough
        )
        self.stale_futures = (
            True  # reset the window index as this is an entry point for the walkthrough
        )
    ...
    def import_file(self, file_path):
        if file_path.endswith(".h5ad"):
            ...
            file_path.lower().endswith(".rds")
            or file_path.lower().endswith(".rda")
            or file_path.lower().endswith(".rdata")
        ):
        ...
        if (
            self.auto_continue
        ):  # set in BioinformaticsWalkthroughWindow_ClusterColumn if the line edit is filled with a column name found in the data
    ...
    def import_file_from_r(self, file_path):
        ...
        reductions_slot_name = (
            None  # only know the slot name for Seurat objects at the moment
        )
        if classname in [
        ...
        try:
            ...
            print(f"Metadata loaded: {self.data_columns}")
    ...
    def search_for_r_spatial_data(self):
        ...
        self.spatial_data_found, key, self.spatial_data = self.search_vis_arrays_for(
            "spatial"
        )
    ...
    def import_file_from_h5ad(self, file_path):
        ...
        try:
            ...
            print(
                f"Import failed while trying to read {file_path} as an anndata object."
            )
            return False
        try:
    ...
    def continue_from_import(self):
        if not self.spatial_data_found:
            ...
            self.open_next_window(
                BioinformaticsWalkthroughWindow_SpatialQuery, show=True
            )
    ...
    def search_columns_for_xyz(self):
        ...
        if x_data_found:
            self.spatial_data_found, _, y_data = self.search_columns_for(
                "y", exact=True
            )
            if not self.spatial_data_found:
                return
            ...
            if z_data_found:
                self.spatial_data = np.hstack(
                    (
                        x_data.values.reshape(-1, 1),
                        y_data.values.reshape(-1, 1),
                        z_data.values.reshape(-1, 1),
                    )
                )
    ...
    def collect_cell_type_data(self):
        ...
        self.cell_types_original = [
            str(x) for x in self.cell_types_original
        ]  # make sure the names are strings
        self.cell_types_list_original = [
            str(x) for x in self.cell_types_list_original
        ]  # make sure the names are strings
        self.remaining_cell_types_list_original = copy.deepcopy(
            self.cell_types_list_original
        )
    ...
    def continue_from_edit(self):
        self.intermediate_types = (
            []
        )  # types used after editing (keep, merge, delete) but before rename
        self.intermediate_type_pre_image = (
            {}
        )  # dictionary where keys are intermediate cell types and values are the original cell types that map to them
        ...
        for cell_type in original_cell_type_list:
            ...
            if intermediate_type is not None:
                if intermediate_type not in self.intermediate_types:
                    ...
                    self.intermediate_type_pre_image[intermediate_type].append(
                        cell_type
                    )
    ...
    def rename_cell_types(self):
        self.open_next_window(
            BioinformaticsWalkthroughWindow_RenameCellTypes, show=True
        )

    def continue_from_rename(self):
        print("-------Continuing from Rename-------")

        if len(set(self.cell_types_list_final)) != len(
            self.cell_types_list_final
        ):  # this very well could be a suboptimal check for all unique names. coded midflight so no copilot help
            duplicate_names = []
            for idx, cell_type_1 in enumerate(self.cell_types_list_final):
                if (cell_type_1 not in duplicate_names) and (
                    cell_type_1 in self.cell_types_list_final[idx + 1 :]
                ):
    ...
    def continue_from_rename_check(self):
        if self.use_spatial_data:
            self.cell_types_final, self.spatial_data_final = zip(
                *[
                    (self.cell_type_dict_on_rename[ctn], pos)
                    for ctn, pos in zip(self.cell_types_original, self.spatial_data)
                    if ctn in self.cell_type_dict_on_rename.keys()
                ]
            )
            self.spatial_data_final = np.vstack([*self.spatial_data_final])
        else:
            self.cell_types_final = [
                self.cell_type_dict_on_rename[ctn]
                for ctn in self.cell_types_original
                if ctn in self.cell_type_dict_on_rename.keys()
            ]
    ...
    def pop_up_warning_window(self, s, continue_cb):
        ...
        hbox.addWidget(label)
        self.open_next_window(
            lambda biwt: BioinformaticsWalkthroughWindow_WarningWindow(
                biwt, hbox, continue_cb
            ),
            show=True,
        )
    ...
    def compute_cell_volumes(self):
        ...
        for cell_type in self.cell_types_list_final:
            if cell_type in self.celldef_tab.param_d.keys():
                self.cell_volume[cell_type] = float(
                    self.celldef_tab.param_d[cell_type]["volume_total"]
                )
            else:
    ...
    def set_cell_positions(self):
        self.open_next_window(
            BioinformaticsWalkthroughWindow_PositionsWindow, show=True
        )
    ...
    def close_up(self):
        ...
        self.window.close()
        print(
            "BioinformaticsWalkthroughWindow: Colors will likely change in the ICs tab due to previous cell types being present."
        )
