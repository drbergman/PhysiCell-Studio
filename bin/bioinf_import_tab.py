...
class BioinfImportWindow(QWidget):
    def __init__(self, bioinf_walkthrough):
        super().__init__()
        self.setWindowTitle(
            f"Bioinformatics Import Walkthrough: Step {bioinf_walkthrough.current_window_idx+1}"
        )
...
class BioinfImportWindow_SpatialQuery(BioinfImportWindow):
    def __init__(self, bioinf_walkthrough):
        ...
        self.continue_button = ContinueButton(
            self, self.biwt.continue_from_spatial_query
        )
...
class BioinfImportWindow_EditCellTypes(BioinfImportWindow):
    def __init__(self, bioinf_walkthrough):
        ...
        row_height = 30
        print(
            f"self.biwt.cell_types_list_original = {self.biwt.cell_types_list_original}"
        )
    ...
    def try_to_plot_dim_red(self, key_substring="umap"):
        found, k, _ = self.biwt.search_adata_obsm_for(
            key_substring
    ...
    def plot_dim_red(self, k):
        ...
        self.scatter = self.dim_red_ax.scatter(
            v[:, 0], v[:, 1], self.marker_size, c=temp.codes
...
class BioinfImportWindow_PositionsWindow(BioinfImportWindow):
    def __init__(self, bioinf_walkthrough):
        ...
        self.ics_plot_area = BioinfImportPlotWindow(
    ...
    def undo_button_cb(self):
        ...
        if self.ics_plot_area.plot_is_2d:
            ...
            self.ics_plot_area.ax0.scatter(
    ...
    def check_for_new_celldefs(self):
        for cell_type in self.biwt.cell_types_list_final:
            if cell_type in self.biwt.celldef_tab.celltypes_list:
                print(
                    f"BioinfImportPlotWindow: {cell_type} found in current list of cell types. Not appending this..."
                )
            else:
...
class BioinfImportPlotWindow(QWidget):
            ...
            if id == 1:
                ...
                self.mpl_cid.append(
                    self.canvas.mpl_connect(
                        "button_release_event", self.mouse_released_cb
                    )
                )  # only appending to history on release; the motion doesn't add to history because it holds focus and so editingFinished signal not emitted while canvas holds focus
                if id == 1:
                    ...
                    self.mpl_cid.append(
                        self.canvas.mpl_connect(
                            "button_press_event", self.disc_mouse_press
                        )
                    )
                    self.mpl_cid.append(
                        self.canvas.mpl_connect(
    ...
    def annulus_plotter(self):
        ...
        try:
            ...
            print(
                "\tBioinfImport WARNING: You likely can use an update to matplotlib to fix a bug in their Annulus plots.\n\tWe'll take care of it for now."
            )
    ...
    def create_figure(self):
        ...
        if self.biwt.use_spatial_data:
            ...
            self.cell_type_pt_area_dict = {
                cell_type: self.fudge_factor
                * self.area_scale_factor
                * 72
                * 72
                / (self.figure.dpi**2)
                * A
            ...
            for cell_type in self.pw.checkbox_dict.keys():
                if self.pw.checkbox_dict[cell_type].isChecked():
                    ...
                    cell_coords = np.hstack(
                        (
                            self.spatial_base_coords[idx_cell_type] * [width, height]
                            + [x0, y0],
    ...
    def plot_cell_pos_single_2d(self, cell_type):
        ...
        self.circles(
            self.new_pos,
            s=(0.75 * self.cell_volume[cell_type] / np.pi) ** (1 / 3),
            color=self.color_by_celltype[cell_type],
            edgecolor="black",
            linewidth=0.5,
            alpha=self.alpha_value,
...
class BioinfImport(QWidget):
    ...
    def open_next_window(self, window_class, layout=None, show=True):
        if self.window is not None:
            ...
            if self.stale_futures:
                print(
                    f"\tFutures are stale. Deleting from {self.current_window_idx} to {len(self.previous_windows)}"
                )
                ...
                print(
                    f"\tFutures are not stale. Using window {self.current_window_idx+1}..."
                )
    ...
    def search_columns_for_xyz(self):
        ...
        if x_data_found:
            ...
            if z_data_found:
                ...
                self.spatial_data[:, 2] += (
                    0.02 * np.random.uniform(size=len(x_data)) - 100
                )
                print(f"\n\nz data found. Adding random z values to avoid overlap.")
                print(f"self.spatial_data[:,2] = {self.spatial_data[:,2]}\n\n\n")
                self.spatial_data_location = (
                    "adata.obs['x'], adata.obs['y'], and adata.obs['z']"
                )
            else:
    ...
    def close_up(self):
        ...
        self.window.close()
        print(
            "BioinfImportWindow: Colors will likely change in the ICs tab due to previous cell types being present."
        )
